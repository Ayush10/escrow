"""Verdict Protocol Console — frontend host + API proxy.

Serves the canonical console and proxies API requests to the demo runner
and judge service backends.
"""
...
app = FastAPI(title="Verdict Protocol Console", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
    ...
    try:
        ...
        return JSONResponse(
            content={
                ...
                "services": {
                    "evidence": os.environ.get("EVIDENCE_SERVICE_URL", "http://127.0.0.1:4001"),
                    "provider": os.environ.get("PROVIDER_API_URL", "http://127.0.0.1:4000"),
                    "runner": RUNNER_URL,
                    "judge": JUDGE_URL,
                    "reputation": os.environ.get("REPUTATION_SERVICE_URL", "http://127.0.0.1:4003"),
                },
            }
        )
