...
contract Court {
    ...
    function _complete(uint256 id, Contract storage c) internal {
    ...
    function timeout(uint256 id) external {
        ...
        address timedOutJudge = d.currentJudge;
        (, uint256 judgeFee, , , , , , uint256 maxResponseTime) = registry.judges(timedOutJudge);
        require(block.timestamp > d.escalatedAt + maxResponseTime, "Judge still has time");

        uint256 slashed = registry.slashBond(timedOutJudge, judgeFee);
        if (slashed > 0 && charity != address(0)) {
            vault.sendExternal(charity, slashed);
        }

        (address superior, , , , , , , ) = registry.judges(timedOutJudge);
        ...
        if (superior == address(0)) {
            ...
            c.clientLocked = 0;
            c.status = Status.Canceled;
            emit SupremeTimeout(id, timedOutJudge);
        } else {
            (, , , , bool active, bool registered, , ) = registry.judges(superior);
            require(registered && active, "Superior not available");
            ...
            d.escalatedAt = block.timestamp;
            _touch(c);
            emit TimedOut(id, timedOutJudge, superior);
        }
    }
